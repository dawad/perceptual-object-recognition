...
/*class Withpoint:public numpunct<char>
{
protected:
...
cout <<"the file "<<file_name<< " have been readed"<<endl;
in.close();
}
...
void Fichiers::read_files(const string &name,const string categorie, vector < const string > &data )
{
std::ifstream in(name.c_str());
std::vector< const string> row;
while(in)
{
	std::string line;
	std::getline(in,line);
	...
	if(categorie=="pos")
	{
	if(strcmp(row.at(1).c_str(),"1")==0 ||strcmp(row.at(1).c_str(),"0")==0)
	{
	data.push_back(row.at(0));
	}
	}
	else 
	if(strcmp(row.at(1).c_str(),"-1")==0 )
	{
	data.push_back(row.at(0));
	}
	row.clear();
...
/// read the list of images of a class 
void Fichiers::read_list(const boost::filesystem3::path & directory, const string classe,const string type,const string categorie, vector<const string>  &files_name)
{
	
	//read_Dirs(directory,files);
	string nom_fichier=directory.string()+classe+"_"+type+".txt";
	read_files(nom_fichier,categorie,files_name);
...
void Fichiers::read_multi_txt_files(const boost::filesystem3::path & directory, std::vector <const string> & descript_files , std::map<std::string ,vector<vector<float>>> & descriptors)
{
	vector <vector<float>> descpt;
	...
	{   
		cout<<" id" <<id<<": ";
		//Fichiers::read_files (descript_files[id].string(), descpt);
		read_txt_file(directory.string()+descript_files[id]+".txt", descpt);
		//string img_name=descript_files[id].substr(0,descript_files[id].size()-13);
		//cout <<"img name " << img_name ;
		
		//if(descriptors.count(descript_files[id])==0)
		//{
		
		descriptors[descript_files[id]]=descpt;
		descpt.clear();
		/*}
		else 
		{			
			descriptors[descript_files[id]].insert(descriptors[descript_files[id]].end(),descpt.begin(),descpt.end());
			descpt.clear();
		}*/
...
/// ecrire le temps de calcul pour kmean pour chaque classe 
	void Fichiers::write_calc_time_kmean(const string &fileName , string classe,string categ,int img_number , float avg_time_kmean, double SIFT_keypoints )
	{
		std::ofstream out(fileName.c_str(),ios::out |ios::app);
		if(!out)
	{
		...
		out<< " SIFT keypoints Total" << SIFT_keypoints<<"\n";
		out.close();
	}
